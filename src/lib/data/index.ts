...
export async function getProductByHandle(
  ...
  const { products } = await medusaRequest("GET", "/products", {
    query: {
      handle,
      sales_channel_id: SALES_CHANNE_ID,
    },
  })
    .then((res) => res.body)
    .catch((err) => {
...
export async function getReviewByProduct(productId: string) {
  try {
    const response = await fetch(
      `${MEDUSA_BE_URL}/store/reviews/${productId}`,
      {
        cache: "no-store",
      }
    )
    ...
    console.log("ERROR::::::: getReviewByProduct", error)
...
export async function createProductReview(data: ReviewData) {
  try {
    const response = await fetch(`${MEDUSA_BE_URL}/store/reviews`, {
      method: "POST",
      headers: {
    ...
    console.log("ERROR::::::: createProductReview", error)
  }
}
